...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    // Validate username
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            // Set parameters
            $param_username = trim($_POST["c_username"]);
    ...
    // Validate email
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            // Set parameters
            $param_email = trim($_POST["c_email"]);
    ...
    if(empty($username_err) && empty($email_err) && empty($confirm_password_err)){
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                mysqli_stmt_close($stmt);
            // Prepare an insert statement for dive center details
                ...
                mysqli_stmt_close($stmt);
                $sql = "INSERT INTO divecenters (user_id, name, address, phone, email) VALUES (?, ?, ?, ?, ?)";
         
                if($stmt = mysqli_prepare($link, $sql)){
                    // Bind variables to the prepared statement as parameters
                    mysqli_stmt_bind_param($stmt, "issss", $param_user_id, $param_name, $param_address, $param_phone, $param_email);
            
                    // Set parameters
                    $param_user_id = $user_id;
                    $param_name = trim($_POST["c_name"]);
                    $param_address = trim($_POST["c_address"]);
                    $param_phone = trim($_POST["c_phone"]);
                    $param_email = $email;
